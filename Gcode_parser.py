...
class machine:
    ...
    def export_gcodeCommands(self):
        """Export the standardized commands, as stored in gcodeCommand instances.
    ...
    def reconstruct_gcode(self):
        """
        Reconstruct raw Gcode commands from processed, standardized gcode Commands.
        
        This method will return all the 'gcodeCommand' instances, which are 
        standardized gcode commands, as raw gcode data. It should return
        functional equivalents of the original raw Gcode command.

        return  :   list (sequence of reconstructed gcode commands)
        """
        
        reconstructedCommands = list()

        moveCommands = ["G0", "G1", "G28", "G92"]
        moveParameters = ["X", "Y", "Z", "E", "F"]                   # loop in orderly fashion over parameters
        
        stateParameters = ["S", "P", "T"]

        # Create an initial lastMachineState, i.e. the first command
        lastMachineState = self.commands[0]

        for cmd in self.commands:
            constructedCommand = ""
            
            # if it's only a comment, return comment and ignore the other if's...
            if cmd.command == "comment":
                constructedCommand += "(" + cmd.parameters["comment"] + ")"
                reconstructedCommands.append(constructedCommand)
                continue
            
            # Check if the command is able to use X,Y,Z,E,F parameters
            if cmd.command in moveCommands:
                constructedCommand+= cmd.command + " "
                for key in moveParameters:
                    if key in cmd.parameters.keys():
                        if cmd.parameters[key] != lastMachineState.parameters[key]:
                            constructedCommand += key + str(cmd.parameters[key]) + " "
            
            # for other commands, add control parameters, if present.
            else:
                constructedCommand += cmd.command + " "
                for key in stateParameters:
                    if key in cmd.parameters.keys():
                        constructedCommand += key + str(cmd.parameters[key]) + " "
            
            # add comment to the end of the line
            if "comment" in cmd.parameters.keys():
                constructedCommand += "(" + cmd.parameters["comment"] + ")"
            
            # remove any trailing whitespace
            constructedCommand.strip()
            reconstructedCommands.append(constructedCommand)

            # update to the latest machine state
            lastMachineState = cmd
...
#    inFile = '/media/data_disk/fablab_utrecht/source/blender-gcode-reader/example files/example_framevertex.gcode'
    ...
    myMachine.reconstruct_gcode()


#    myList = myMachine.export_gcodeCommands()                                   # export to list
